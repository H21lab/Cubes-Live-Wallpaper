...
class M3DM {
	...
	ZSORTstruct zsort[] = new ZSORTstruct[MAXOBJ];
	int Count_zsort = 0;
	...
	private final String vertexShaderCode = 
	        ...
	        //" vec4 lightPos = uVMatrix * uLightPos; \n" +
	        " vLightPos = vec3(uLightPos.x, uLightPos.y, uLightPos.z); \n" +
	        //"  vLight = max(dot(aNormal, uLightPos), 0.0) + 0.2; \n" +
	        "}                         \n";
	...
	void initializeGL() {
        ...
        GLES20.glLinkProgram(glProgram);                  // creates OpenGL program executables
	...
	//	Matrix.frustumM(projectionMatrix, 0, -P_NPlane*(float)Math.tan(P_fov_horiz/2.0f), P_NPlane*(float)Math.tan(P_fov_horiz/2.0f), -P_NPlane*(float)Math.tan(P_fov_vert/2.0f), P_NPlane*(float)Math.tan(P_fov_vert/2.0f), P_NPlane, P_FPlane);
	...
	static class mD3DMesh {
		...
		mD3DMesh(int VertexType, float V[], short D[]) {
			int strsize = 0;
			if ((VertexType & M3DFVF_XYZ) == M3DFVF_XYZ) {
				strsize += 3;
			}
			if ((VertexType & M3DFVF_NORMAL) == M3DFVF_NORMAL) {
				strsize += 3;
			}
			if ((VertexType & M3DFVF_TEX1) == M3DFVF_TEX1) {
				strsize += 2;
			}
			if ((VertexType & M3DFVF_COLOR) == M3DFVF_COLOR) {
				strsize += 4;
			}
			VertexSize = strsize;
			Flags = 0;
			...
			int i = 0, j = 0, Vnum = 0;
			while (D[i] != 0) {
				// calculation of VertexCount according to receive data
				for (j = i + 1; j < D[i] + i + 1; j++) {
					if (Vnum < D[j]) {
						Vnum = D[j];
					}
				}
			...
			VertexStruct = VertexType;
			if (VertexCount > 0) {
				Vertex = new float[VertexCount * strsize];
				for (j = 0; j < VertexCount * strsize; j++) {
					Vertex[j] = V[j];
				}
			}
			Material = new M3DMATERIAL();
		...
		mD3DMesh(M3DVERTEX_RGB V[], short D[]) {
			int strsize = 12;
			VertexSize = strsize;
			Flags = 0;
			Textures = 0;
			...
			VertexStruct = M3DFVF_XYZ | M3DFVF_NORMAL | M3DFVF_TEX1 | M3DFVF_COLOR;
			if (VertexCount > 0) {
				Vertex = new float[VertexCount * strsize];
				for (j = 0; j < VertexCount; j++) {
					...
					Vertex[j * strsize + 8] = V[j].r;
					Vertex[j * strsize + 9] = V[j].g;
					Vertex[j * strsize + 10] = V[j].b;
					Vertex[j * strsize + 11] = V[j].a;
				}
			}
			Material = new M3DMATERIAL();
		}

		mD3DMesh(M3DVERTEX V[], short D[]) {
			int strsize = 8;
			VertexSize = strsize;
			Flags = 0;
			Textures = 0;
	...
	private void _renderMesh(mD3DMesh mesh, M3DMATRIX world, int rendersubmesh) {
        ...
        GLES20.glUniformMatrix4fv(GLES20.glGetUniformLocation(glProgram, "uMVPMatrix"), 1, false, mMVPMatrix, 0);
        GLES20.glUniformMatrix4fv(GLES20.glGetUniformLocation(glProgram, "uMVMatrix"), 1, false, mMVMatrix, 0);
        GLES20.glUniformMatrix4fv(GLES20.glGetUniformLocation(glProgram, "uVMatrix"), 1, false, mVMatrix, 0);
		...
		/*for (int q = 0; q < 8; q++) {
			if (q < mesh.Textures) {
				setTexture(q, mesh.Texture[q]);
			} else {
				setTexture(q, new mD3DTexture());
			}
		}*/
		...
		if ((mesh.VertexCount > 0) && ((mesh.VertexStruct & M3DFVF_XYZ) == M3DFVF_XYZ)) {
	        ...
	        GLES20.glEnableVertexAttribArray(GLES20.glGetAttribLocation(glProgram, "aPosition"));
	        GLES20.glVertexAttribPointer(GLES20.glGetAttribLocation(glProgram, "aPosition"), 3, GLES20.GL_FLOAT, false, mesh.VertexSize*4, vertexBuffer);
	       
	        vertexBuffer.position(3);
	        GLES20.glEnableVertexAttribArray(GLES20.glGetAttribLocation(glProgram, "aNormal"));
	        GLES20.glVertexAttribPointer(GLES20.glGetAttribLocation(glProgram, "aNormal"), 3, GLES20.GL_FLOAT, false, mesh.VertexSize*4, vertexBuffer);
		    
	        vertexBuffer.position(6);
	        GLES20.glEnableVertexAttribArray(GLES20.glGetAttribLocation(glProgram, "aTex"));
	        GLES20.glVertexAttribPointer(GLES20.glGetAttribLocation(glProgram, "aTex"), 2, GLES20.GL_FLOAT, false, mesh.VertexSize*4, vertexBuffer);
		    
	        // Light
	        if (N_Lights > 0) {
	        	GLES20.glUniform4f(GLES20.glGetUniformLocation(glProgram, "uLightPos"), mTransformedLightVector[0], mTransformedLightVector[1], mTransformedLightVector[2], mTransformedLightVector[3]);
	        	
	        	GLES20.glUniform4f(GLES20.glGetUniformLocation(glProgram, "uLightAmbientColor"), Light[0].AR, Light[0].AG, Light[0].AB , Light[0].AA );
		        GLES20.glUniform4f(GLES20.glGetUniformLocation(glProgram, "uLightDiffuseColor"), Light[0].DR, Light[0].DG, Light[0].DB , Light[0].DA );
		        GLES20.glUniform4f(GLES20.glGetUniformLocation(glProgram, "uLightSpecularColor"), Light[0].SR, Light[0].SG, Light[0].SB , Light[0].SA );
		        GLES20.glUniform1f(GLES20.glGetUniformLocation(glProgram, "uLightAttenuation"), Light[0].AT );
	        }
	        
	        // Material
	    	GLES20.glUniform4f(GLES20.glGetUniformLocation(glProgram, "uMaterialAmbientColor"), mesh.Material.AR, mesh.Material.AG, mesh.Material.AB , mesh.Material.AA );
	    	GLES20.glUniform4f(GLES20.glGetUniformLocation(glProgram, "uMaterialDiffuseColor"), mesh.Material.DR, mesh.Material.DG, mesh.Material.DB , mesh.Material.DA );
	    	GLES20.glUniform4f(GLES20.glGetUniformLocation(glProgram, "uMaterialSpecularColor"), mesh.Material.SR, mesh.Material.SG, mesh.Material.SB , mesh.Material.SA );
	    	GLES20.glUniform4f(GLES20.glGetUniformLocation(glProgram, "uMaterialEmissiveColor"), mesh.Material.ER, mesh.Material.EG, mesh.Material.EB , mesh.Material.EA );
	    	GLES20.glUniform1f(GLES20.glGetUniformLocation(glProgram, "uMaterialShinnes"), mesh.Material.SH );
			...
			while (mesh.Data[i] != 0) {
				...
				short D[] = new short[mesh.Data[i]];
				for (int j = 0; j < mesh.Data[i]; j++) {
					D[j] = mesh.Data[i + 1 + j];
				}
			
		        // draw	        
		        ShortBuffer indexBuffer = ShortBuffer.allocate(D.length);
		        indexBuffer.put(D);
		        indexBuffer.position(0);
		        GLES20.glDrawElements(GLES20.GL_TRIANGLE_FAN, D.length, GLES20.GL_UNSIGNED_SHORT, indexBuffer);
	...
	private void _renderFrame(mD3DFrame frame, M3DMATRIX world_up) {
		...
		for (i = 0; i < frame.Meshs; i++) {
			// if(ZSORT==1){_RenderMesh_Zsort(frame.Mesh[i], actual);}
			// else{_RenderMesh(frame.Mesh[i], actual, 1);}
	...
	void renderFrame(mD3DFrame frame) {
		...
		// if(ZSORT==1){Count_zsort=0;}
		_renderFrame(frame, M3DMATRIX.IdentityMatrix());
		// if(ZSORT==1){ATTEMPT(_RenderFromBuff_zsort());}
	}
	...
	static mD3DMesh createSphere(float R, int StepRov, int StepPol, float U1, float V1, float U2, float V2) {
		...
		for (j = 0; j < NPoludnikov - 1; j++) {
			for (i = 0; i < NRovnobeziek - 1; i++) {
				if (i == 0) {
					data[count] = 3;
					data[count + 3] = (short)(i + 1 + j * NRovnobeziek);
					data[count + 2] = (short)(i + j * NRovnobeziek);
					data[count + 1] = (short)(i + 1 + (j + 1) * NRovnobeziek);
					count += 4;
				} else if (i == NRovnobeziek - 2) {
					data[count] = 3;
					data[count + 3] = (short)(i + 1 + j * NRovnobeziek);
					data[count + 2] = (short)(i + j * NRovnobeziek);
					data[count + 1] = (short)(i + (j + 1) * NRovnobeziek);
					count += 4;
				} else {
					data[count] = 4;
					data[count + 4] = (short)(i + 1 + j * NRovnobeziek);
					data[count + 3] = (short)(i + j * NRovnobeziek);
					data[count + 2] = (short)(i + (j + 1) * NRovnobeziek);
					data[count + 1] = (short)(i + 1 + (j + 1) * NRovnobeziek);
					count += 5;
				}
			}
	...
	// Method culculates the x, y position on screen into line in 3D space. Set as output POINT and VECT
